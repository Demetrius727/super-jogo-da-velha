      ...
      <div className="status">
        {winner ? (
          <div>
            <p
              className={`winner ${
                winner === "X" ? "winner-player" : "winner-ai"
              }`}
            >
              O vencedor é: {winner}!
            </p>
      ...
      <div>
        {Array.from({ length: boardSize }).map((_, rowIndex) => (
          <div className="board-row" key={rowIndex}>
            {squares
              .slice(rowIndex * boardSize, rowIndex * boardSize + boardSize)
              .map((square, colIndex) => {
...
// Todas as combinações buscadas pra uma vitória, também usada pra IA determinar suas escolhas
...
const randomAiMove = (squares, setSquares, setIsNext) => {
  ...
  const availableCenters = centerIndices.filter(
    (index) => squares[index] === null
  );

  if (availableCenters.length > 0 && Math.random() < 0.5) {
    const randomCenter =
      availableCenters[Math.floor(Math.random() * availableCenters.length)];
